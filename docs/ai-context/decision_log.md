...
## 파일 시스템 브라우징
### 검색 결과 키워드 하이라이트 적용 (#166, 2026-03-01)
- **상황**:
  - 검색 기능(`#122`)의 마지막 미완료 항목으로, 헤더 즉시결과와 `/search` 결과에서 검색어가 시각적으로 구분되지 않아 스캔 속도가 느렸다.
  - `/search`는 테이블/그리드 뷰를 모두 지원하므로 특정 뷰만 보강하면 UX가 다시 불일치할 수 있었다.
- **결정**:
  - 검색어 하이라이트 공통 유틸 `highlightQueryMatch`를 추가해 문자열 분할 + `<mark>` 렌더를 일관화한다.
  - 헤더 검색 드롭다운(`MainLayout`)과 `/search` 결과(`FolderContent` 검색 모드) 모두 동일 유틸을 사용한다.
  - `FolderContentTable`/`FolderContentGrid`에 `renderName` 확장 포인트를 추가하고, 검색 모드에서만 `renderSearchName`을 주입한다.
  - 하이라이트 색상은 전역 클래스(`search-match-highlight`)로 정의해 테마 토큰 기반으로 관리한다.
- **이유**:
  - 검색 결과 표현 규칙을 공통화하면 동일 키워드에 대한 인지 경험이 헤더/페이지/뷰모드 전반에서 일관된다.
  - `renderName` 주입 방식은 기존 데이터 모델(`FileNode.name: string`)을 유지하면서 검색 모드 전용 렌더 요구만 분리할 수 있어 회귀 리스크가 낮다.
...
### FolderTree 초기화 의존성 분리 (2026-02-25)
- **문제**:
  - `/trash`에서 스페이스가 0개일 때 `TrashExplorer`의 반복 `fetchSpaces()`가 발생하고,
  - `FolderTree(showBaseDirectories=true)` 초기화가 `spaces` 변경에 연동되어 모달 트리 확장 상태가 반복 리셋됐다.
- **결정**:
  - `TrashExplorer`에서 빈 스페이스 상태 시 `fetchSpaces` 반복 호출을 제거한다.
  - `FolderTree` 초기화를 두 effect로 분리한다.
    - 시스템 디렉터리 모드(`showBaseDirectories`) 전용 초기화
    - 일반 스페이스/단일 스페이스 트리 전용 초기화
  - 시스템 디렉터리 모드 초기화는 `spaces` 변경에 의존하지 않도록 고정한다.
- **이유**:
  - 스페이스 목록 갱신과 모달용 시스템 트리 초기화를 분리해야, 무관한 상태 변경으로 트리 확장 상태가 초기화되는 UX 회귀를 막을 수 있다.

### 휴지통 모드의 트리 재선택 처리 정책 (2026-02-25)
- **문제**:
  - `/trash` 진입 시 `selectedSpace/selectedPath` 상태는 유지되며, 사용자가 같은 Space를 다시 클릭하면 `FolderTree`의 same-selection guard에 의해 `onSelect`가 생략됐다.
  - 그 결과 `MainLayout`의 `/` 복귀 내비게이션이 실행되지 않아 “휴지통에서 Space 클릭 전환 불가” UX가 발생했다.
- **결정**:
  - `FolderTree`에 `bypassSameSelectionGuard` 옵션을 추가한다.
  - `MainSider`는 검색/휴지통 모드에서 해당 옵션을 활성화해, 같은 노드 재선택이어도 `onSelect`를 전달한다.
  - `TrashExplorer`는 Space 0개 상태에서 조기 반환하지 않고 기본 레이아웃을 유지하며, empty 문구만 분기한다.
- **이유**:
...
### WebDAV 루트 라우트 이중 등록으로 메서드 보존 (2026-02-25)
- **문제**:
  - 서버가 `/dav/`만 등록된 상태에서 `/dav` 요청이 `301` 리다이렉트됐다.
  - 일부 DAV 클라이언트는 리다이렉트 후 원래 메서드(`PROPFIND`/`OPTIONS`)를 보존하지 않고 `GET`으로 재요청해 루트 디렉터리에서 `405 Method Not Allowed`를 유발했다.
- **결정**:
  - WebDAV 핸들러를 `/dav`와 `/dav/` 경로 모두에 직접 등록해 리다이렉트를 제거한다.
- **이유**:
  - 루트 연결 시 클라이언트별 리다이렉트/메서드 처리 차이를 서버에서 흡수해, Space 목록 조회(루트 PROPFIND)를 안정적으로 보장하기 위함이다.

### WebDAV OPTIONS 무인증 허용으로 연결 핸드셰이크 안정화 (2026-02-26)
- **문제**:
  - WebDAV 핸들러가 모든 메서드에서 Basic Auth를 선행 검사해 `OPTIONS`도 401로 차단됐다.
  - 일부 클라이언트 환경에서는 OPTIONS 단계에서 연결 협상이 실패해 사용자 입장에서 자격증명 입력 기회 없이 차단되는 UX가 발생했다.
- **결정**:
  - 루트 경로(`OPTIONS /dav`, `OPTIONS /dav/`)만 무인증 `200`으로 허용한다.
  - 루트 OPTIONS 응답은 WebDAV root handler로 위임해 capability 헤더를 동적으로 반환한다.
  - 하위 경로 `OPTIONS`와 실제 데이터 접근 메서드(`PROPFIND/GET/PUT/COPY/MOVE...`)는 기존처럼 Basic Auth와 Space 권한 검증을 유지한다.
- **이유**:
  - 연결 협상(능력 탐지)과 리소스 접근(권한 검증)을 분리해야 다양한 WebDAV 클라이언트에서 초기 연결 성공률을 높일 수 있다.

### DB 재생성 이후 stale JWT 차단을 위한 사용자 실체 검증 추가 (2026-02-25)
- **문제**:
  - JWT는 서명/만료만 유효하면 통과해, DB 삭제/재생성 이후에도 기존 세션이 일부 API를 호출할 수 있었다.
  - 그 결과 역할 기반 엔드포인트는 통과하고, space 리소스 권한 엔드포인트는 실패하는 불일치 상태가 발생했다.
- **결정**:
  - 인증 미들웨어에서 토큰 파싱 직후 `user_id + username(+subject)` 기준으로 현재 DB 사용자와 일치 여부를 검증한다.
  - 검증 성공 시 claims를 현재 DB 사용자 정보(닉네임/역할 포함)로 동기화해 stale 클레임 사용을 최소화한다.
  - refresh 토큰 처리에도 동일 검증을 적용한다.
  - 사용자 `created_at`이 토큰 `issuedAt`보다 늦은 경우(초 단위) 토큰을 무효 처리한다.
- **이유**:
  - 계정 실체가 사라졌거나 재생성된 상황에서 과거 토큰으로 인증이 유지되는 보안/UX 문제를 차단할 수 있다.
  - 인증 실패를 일관된 401로 정리해 프론트의 세션 만료 처리(로그인 리다이렉트) 동선을 정상화할 수 있다.

### 실행파일 루트 로그 디렉터리(`logs/`) 표준화 (2026-02-25)
- **문제**:
  - 터미널/백그라운드 실행 방식이 혼재되어 앱 로그와 업데이터 로그 확인 경로가 일관되지 않았다.
  - 업데이트 후 재시작 프로세스는 터미널과 분리되어 표준출력 로그 추적이 어려웠다.
- **결정**:
  - 서버는 실행파일 루트 기준 `logs/app.log`를 생성하고 append로 기록한다.
  - 업데이터는 실행파일 루트 기준 `logs/updater.log`를 생성하고 append로 기록한다.
  - 업데이터가 재시작한 앱 프로세스의 `stdout/stderr`를 `logs/app.log`로 리다이렉트한다.
- **이유**:
  - 실행 방식과 무관하게 로그 위치를 고정하면 운영자가 장애 시점의 앱/업데이터 흐름을 단일 디렉터리에서 추적할 수 있다.

### PID 파일 기반 종료 스크립트 제공 (2026-02-25)
- **문제**:
  - 백그라운드 실행/업데이트 재시작 이후에는 기존 터미널에서 `Ctrl+C`로 서버를 제어하기 어려웠다.
- **결정**:
  - 서버 프로세스가 실행파일 루트에 `cohesion.pid`를 기록하도록 한다.
  - 릴리즈 아카이브에 PID 기반 종료 스크립트를 포함한다.
    - `stop-cohesion.command` (macOS/Linux)
    - `stop-cohesion.bat` (Windows)
- **이유**:
  - 앱 번들/서비스 매니저 없이도 사용자가 배포 폴더 단위로 서버 종료를 직접 제어할 수 있다.

## 개발 프로세스
### FolderContent 책임 분리 2차: 검색/히스토리 로직 훅 추출 (2026-03-01)
- **문제**:
  - `FolderContent.tsx`가 검색 모드 브릿지와 경로 히스토리 상태를 동시에 직접 관리해, 파일 크기와 책임 결합도가 커지고 후속 기능(#166/#167) 적용 시 회귀 검증 범위가 넓어졌다.
- **결정**:
  - 검색 모드 전환 로직을 `useSearchModeContent` 훅으로 추출한다.
    - 검색 결과 `SearchFileResult -> FileNode` 매핑
    - rowPath 기반 결과 조회/열기
    - 검색 메타 텍스트 생성
    - 검색/브라우즈 모드별 `activeLoading`/`activeErrorMessage` 계산
  - 폴더 히스토리 이전/다음 로직을 `useBrowseHistoryNavigation` 훅으로 추출한다.
    - space 전환 시 히스토리 초기화
    - 히스토리 탐색 중 상태 플래그 처리
    - 검색 복귀(`fromSearchQuery`) fallback 처리
- **이유**:
  - 상태 계산/이벤트 처리 책임을 컴포넌트 밖으로 이동하면 `FolderContent`는 조립과 렌더 중심으로 단순화되고, 후속 기능 개발 시 변경 영향 범위를 더 좁게 통제할 수 있다.

### FolderContent 책임 분리 2차 보강: 휴지통 모달 훅 추출 + 스코프 고정 (2026-03-01)
- **문제**:
  - `FolderContent.tsx` 내 휴지통 모달 상태/확인 다이얼로그/복원·삭제·비우기 액션이 컴포넌트 내부에 결합돼 책임 분리 목표가 부분 달성 상태였다.
  - 리팩토링 이슈(`#165`) 작업 중 검색 하이라이트(`#166`) 변경이 섞여 스코프 경계가 흐려졌다.
- **결정**:
  - 휴지통 전용 상태와 액션을 `useTrashModalManager` 훅으로 추출한다.
    - 모달 open/close
    - 목록 로딩/처리 상태
    - 선택 ID 관리
    - 복원/영구삭제/비우기 confirm 플로우
  - `#165`에서 하이라이트 관련 변경을 제거해 리팩토링 범위만 유지한다.
- **이유**:
  - 모달 제어 로직을 훅으로 분리하면 `FolderContent`는 파일 탐색 상호작용 조립에 집중할 수 있어 유지보수성이 올라간다.
  - 이슈 단위 스코프를 고정해야 AC 판정과 회귀 분석이 명확해지고 리뷰 비용이 낮아진다.

### 다음 개발 우선순위 기반 이슈 등록 (2026-02-28)
- **결정**:
  - 사용자 요청 우선순위 `4 -> 2 -> 3` 순서로 후속 개발 이슈를 분리 등록한다.
  - 이슈 매핑:
    - `#165`: `FolderContent.tsx` 책임 분리 리팩토링 2차
    - `#166`: 검색 결과 키워드 하이라이트
    - `#167`: 텍스트 파일 미리보기(읽기 전용)
- **이유**:
  - `#165`를 선행해 탐색기 코어 컴포넌트 결합도를 낮추면, `#166/#167` 적용 시 회귀 범위와 리뷰 복잡도를 줄일 수 있다.
  - `#166`은 기존 검색 기능의 가독성 문제를 작은 변경으로 개선하는 빠른 UX 개선 항목이다.
  - `#167`은 사용자 체감 가치가 높지만 API/권한/용량 제한 정책이 필요해 별도 기능 이슈로 독립 관리가 적합하다.

### WebDAV PROPFIND Depth 상한 정책 도입 (2026-02-26)
- **문제**:
  - WebDAV 클라이언트가 `Depth: infinity` 또는 Depth 생략 상태로 `PROPFIND`를 보내면, 서버가 디렉터리 하위 전체를 재귀 조회해 고비용 I/O를 유발할 수 있다.
- **결정**:
  - 인증 이후 `PROPFIND` 요청의 Depth를 서버에서 정규화한다.
    - `Depth: 0`, `Depth: 1`은 그대로 허용
    - 그 외 값(`infinity`/생략/비정상 값)은 `Depth: 1`로 강제
- **이유**:
  - 네트워크 드라이브 호환성(폴더 단위 탐색)은 유지하면서, 전체 트리 재귀 스캔으로 인한 과부하 위험을 즉시 낮추기 위함이다.

### GitHub Actions Go 캐시 의존 파일 경로 고정 (2026-02-26)
- **문제**:
  - `actions/setup-go` 캐시가 기본값(루트 `go.sum`)을 찾는데, 모노레포 구조상 실제 `go.sum`은 `apps/backend/go.sum`에 위치한다.
  - 릴리즈 파이프라인에서 캐시 복원 실패 메시지가 발생해 워크플로 안정성이 저하될 수 있다.
- **결정**:
  - CI/Release 워크플로 모두 `Setup Go` 단계에 `cache-dependency-path: apps/backend/go.sum`을 명시한다.
- **이유**:
  - 모노레포 경로 구조를 명시적으로 반영해 캐시 복원 실패를 제거하고, 파이프라인 신뢰도를 높이기 위함이다.

### 자동 업데이트 1차는 본체 내 교체 대신 별도 업데이터 프로세스로 분리 (2026-02-24)
- **문제**:
  - 실행 중인 본체 바이너리를 동일 프로세스에서 교체하면 OS별 파일 잠금(특히 Windows)과 롤백 처리 복잡도가 커진다.
- **결정**:
  - 본체(`cohesion`)는 업데이트 준비(릴리즈 조회/다운로드/검증/추출)와 종료 신호까지만 담당한다.
  - 실제 바이너리 교체/재실행은 별도 바이너리(`cohesion-updater`)가 수행한다.
  - 릴리즈에는 본체와 업데이터를 함께 포함하고, 본체는 실행 파일 디렉토리의 업데이터를 호출한다.
  - 무결성 검증은 GitHub 릴리즈의 `checksums.txt`를 기준으로 SHA-256 비교를 강제한다.
- **이유**:
  - 업데이트 도중 본체 프로세스가 죽거나 교체가 실패해도, 교체 책임을 분리하면 복구/재시도 경로가 단순해진다.
  - 플랫폼별 잠금 제약을 업데이터 내부 재시도 로직으로 격리해 본체 API/런타임 안정성을 높일 수 있다.

### 자동 업데이트 테스트를 위한 동일 버전 강제 재설치(`force`) 허용 (2026-02-24)
- **문제**:
  - 업데이트 기능 검증 시, 실제 최신 버전 환경에서는 `isNewerVersion=false`로 인해 업데이트 플로우를 재현하기 어렵다.
- **결정**:
  - `POST /api/system/update/start`에 `force` 쿼리(`?force=1`)를 추가해 동일 버전도 재설치 가능하게 한다.
  - About 버튼은 `updateAvailable=false`일 때 자동으로 `force` 모드로 호출한다.
- **이유**:
  - QA/수동 점검에서 실제 릴리즈를 기다리지 않고도 다운로드·검증·교체·재시작 전체 경로를 반복 검증할 수 있다.

### 자동 업데이트 안정화 보완: 권한/동시성/정리 처리 (2026-02-24)
- **문제**:
  - 업데이트 API 권한 매핑 누락으로 서버 권한이 없는 계정도 호출 가능한 경로가 존재했다.
  - `switching` 직전 `running=false` 처리로 중복 업데이트 요청이 경쟁적으로 들어올 여지가 있었다.
  - 업데이트 임시 디렉토리 정리 책임이 불명확해 누적 가능성이 있었다.
- **결정**:
  - `/api/system/update/status`는 `server.config.read`, `/api/system/update/start`는 `server.config.write` 권한으로 매핑한다.
  - `switching` 상태에서는 `running`을 유지해 중복 `start`를 차단한다.
  - 임시 디렉토리는 업데이터가 `--cleanup-dir` 인자로 넘겨받아 작업 종료 시 정리한다.
- **이유**:
  - 서버 제어성(업데이트/재시작)에 맞는 최소 권한 원칙을 유지하고,
  - 업데이트 전환 구간의 경쟁조건을 제거하며,
  - 장기 운영 시 임시파일 누적 리스크를 줄이기 위함.

### Settings에 About 섹션을 별도 메뉴로 분리 (2026-02-24)
- **문제**:
  - 업데이트 체크/버전 메타데이터가 도입되었지만, 설정 화면에 제품 소개와 버전 정보를 일관되게 확인할 전용 영역이 없었다.
- **결정**:
  - Settings 좌측 메뉴에 `About` 항목을 별도로 추가한다.
  - `About` 화면은 두 개의 카드로 구성한다.
    - `Cohesion` 카드: 제품 설명 + GitHub 링크
    - `버전 정보` 카드: 현재 버전 + 최신 버전 + 릴리즈 링크
  - 현재 버전은 우선순위 `update-check.currentVersion -> /api/system/version -> dev`로 표시한다.
- **이유**:
  - 서버 상태 팝오버와 설정 정보 영역의 역할을 분리해, 운영 정보(상태)와 제품 정보(버전/출처)를 각각 찾기 쉽게 만들기 위함.
  - 향후 업데이트 액션(수동 다운로드/외부 업데이터 진입)을 `About` 화면에 일관되게 확장할 기반을 확보하기 위함.

### 릴리즈 업데이트 전략 1차: 앱 내 알림 + 수동 업데이트 유지 (#146, 2026-02-24)
- **문제**:
  - 릴리즈는 GitHub 아티팩트로 배포되지만, 실행 중 앱에서 최신 버전 존재 여부를 사용자에게 알릴 경로가 없었다.
  - 곧바로 자가 업데이트를 도입하면 OS별 바이너리 교체/권한/롤백 복잡도가 커진다.
- **결정**:
  - 1차 범위를 `버전 식별 + 최신 릴리즈 비교 + UI 알림`으로 제한한다.
  - 백엔드에 `GET /api/system/version`, `GET /api/system/update-check`를 추가한다.
  - 업데이트 체크는 GitHub latest release API를 사용하고, 서버 메모리 캐시(TTL)로 호출 빈도를 제한한다.
  - 조회 실패는 API 5xx 대신 `error` 필드가 포함된 정상 응답으로 처리해 UI를 깨뜨리지 않는다.
  - 프론트는 상태 팝오버에 업데이트 섹션/릴리즈 링크를 노출하고, 새 버전이 있을 때 헤더에 배지만 표시한다.
- **이유**:
  - 사용자에게 즉시 가치(업데이트 인지)를 제공하면서 운영 리스크(자가 업데이트 실패/롤백 문제)를 낮출 수 있다.
  - 2차(외부 업데이터/자동 교체) 설계를 위한 버전 메타데이터 기반을 먼저 확보할 수 있다.

### 설정 UI 미사용 항목 제거 기준 확정 (2026-02-24)
- **문제**:
  - 설정 화면에 실사용되지 않는 옵션이 노출되어 사용자 기대와 실제 동작이 불일치했다.
  - 특히 파일/캐시/기본뷰 관련 설정은 사용자에게 “적용된다”는 인상을 주지만 현재 제품 동작과 연결되지 않았다.
- **결정**:
  - 설정 메뉴에서 `파일` 섹션을 제거한다.
  - `일반` 섹션의 `캐시 삭제` 액션을 제거한다.
  - `외관` 섹션의 `기본 뷰 모드` 옵션을 제거한다.
  - 프론트 `settingsStore`에서 해당 미사용 필드/액션을 함께 제거해 상태 모델을 단순화한다.
- **이유**:
  - UX 관점에서 “보이는 설정은 즉시 의미가 있어야 한다”는 원칙을 우선 적용해 혼선을 줄인다.
  - 미사용 상태 모델을 정리하면 향후 설정 확장 시 회귀 범위와 유지보수 비용을 줄일 수 있다.
...
### 파일 익스플로러 입력 모드 분리: 해상도와 상호작용 기준 분리 (2026-02-24)
- **문제**:
  - 파일 익스플로러의 입력 분기가 `isMobile(해상도)` 기준에 강하게 묶여 있어, PC 해상도의 터치 입력 환경에서 마우스 규칙과 터치 제스처가 어색하게 섞였다.
- **결정**:
  - `layoutMode`(desktop/mobile)와 `interactionMode`(pointer/touch)를 분리한다.
  - 레이아웃 관련(행 높이/배치)은 `layoutMode`만 사용하고, 선택/탭/컨텍스트메뉴/드래그/박스선택은 `interactionMode`로 분기한다.
  - 터치 입력 가능 환경(`maxTouchPoints` 또는 `any-pointer: coarse`)에서는 모바일 상호작용 규칙을 적용한다.
- **이유**:
  - 입력 장치 특성에 맞춘 분리 없이는 동일 화면폭에서도 상호작용 일관성이 깨지며, 특히 태블릿/터치 노트북에서 오동작 체감이 커지기 때문이다.

### lint baseline 데이터 경고 처리 정책 (2026-02-23)
- **문제**:
  - `pnpm -C apps/frontend lint` 실행 시 `baseline-browser-mapping` 데이터가 오래되었다는 경고가 반복 출력되었다.
- **결정**:
  - `apps/frontend`에 `baseline-browser-mapping`을 최신 버전으로 devDependency 명시 추가한다.
- **이유**:
  - 린트 실패는 아니지만 브라우저 기준 데이터가 구버전일 수 있어, 불필요 경고를 제거하고 lint 출력 신뢰도를 높이기 위함.

### 메인 헤더 브랜드 링크 및 `/api/config` 유효성 검증 강화 (#139, 2026-02-23)
- **문제**:
  - 메인/검색/휴지통 레이아웃의 헤더 브랜드(`Cohesion`)가 클릭 가능한 홈 이동 동선이 없어 내비게이션 일관성이 떨어졌다.
  - 서버 설정 저장 API(`/api/config`)가 `server.port` 비어 있음만 검사해, 잘못된 포트/포트 충돌 설정이 저장될 수 있었다.
- **결정**:
  - `MainLayout`의 헤더 브랜드를 클릭 시 `/`로 이동하도록 연결한다.
  - 프론트 `ServerSettings`에 저장/재시작 전 동일 기준의 사전 검증을 추가한다.
    - `server.port`: 정수, 1~65535
    - `sftpEnabled=true`: `sftpPort` 1~65535 및 `server.port`와 충돌 금지
  - 백엔드 `/api/config`에 동일 규칙의 최종 검증을 추가하고, 단위 테스트를 추가한다.
  - 프론트 `updateConfig`는 실패 응답(`error`/`message`)을 파싱해 사용자에게 구체 오류를 노출한다.
- **이유**:
  - 클라이언트 사전 검증 + 서버 최종 검증을 동시에 적용해야 우회 요청과 UI 오류를 모두 방지할 수 있다.
  - 브랜드 홈 이동은 메인/설정 레이아웃의 탐색 규칙을 통일해 사용성 회귀를 줄인다.
...
- **결정**:
  - switcher 관련 공통 규칙 셀렉터를 `.folder-tree:not(.layout-sider-tree-action)`로 제한해, 휴지통 액션 트리에서는 적용되지 않도록 분리한다.
  - 휴지통 라벨 아이콘도 `DeleteOutlined` 대신 `material-symbols-rounded(delete)`로 교체해 트리와 동일 아이콘 계열을 사용한다.
  - 휴지통 라벨 래퍼(`inline-flex`)의 baseline 정렬로 생기던 행 높이 증가(29px)를 `line-height: 24px` + `vertical-align: top`으로 보정한다.
  - 휴지통 행의 아이콘/텍스트 시작점이 스페이스 트리와 다르지 않도록, 라벨 래퍼를 `margin-left: -8px`, `gap: 12px`으로 조정한다.
- **이유**:
  - `Tree` 기반 선택/포커스 스타일 일관성은 유지하면서, 휴지통 항목의 불필요한 좌측 예약 폭/아이콘 언어/행 높이 불일치를 한 번에 정합화할 수 있기 때문이다.

### 휴지통 수동 새로고침 버튼 제거 (2026-02-23)
- **문제**:
  - 휴지통 화면(`/trash`)과 기존 휴지통 모달에 `새로고침` 버튼이 남아 있어, 자동 갱신 기반 동선과 중복되고 상단 액션 밀도가 불필요하게 높았다.
- **결정**:
  - 휴지통 화면과 모달 footer에서 `새로고침` 버튼을 제거한다.
  - 모달 컴포넌트의 `onRefresh` prop도 함께 제거해 API 표면을 정리한다.
- **이유**:
  - 복원/영구삭제/비우기 이후에는 이미 목록 재조회가 수행되고, 수동 새로고침 액션은 중복 기능이라 UI 단순화 효과가 더 크기 때문이다.

### 휴지통 진입점을 Footer 버튼에서 트리 하단 트리 스타일로 전환 (2026-02-23)
- **문제**:
  - 휴지통이 `text button` 기반 footer 액션이라, 스페이스 트리(`Tree`) 선택 항목과 시각 언어가 달라 PC/모바일에서 이질감이 발생했다.
- **결정**:
  - `SidePanelShell` footer 버튼을 제거하고, 트리 본문 하단에 별도 `Tree.DirectoryTree`(단일 `휴지통` 항목)를 배치한다.
  - 휴지통 항목은 최하단 고정이 아니라 스페이스 트리 바로 아래 같은 스크롤 문맥으로 배치한다.
  - 휴지통 항목의 구분선은 제거하고, leaf switcher placeholder를 숨겨 트리 라벨과 아이콘 좌측 여백을 축소한다.
- **이유**:
  - 동일 컴포넌트(`Tree`)를 사용하면 selected/hover/focus 토큰이 일관되어 플랫폼(PC/모바일 Drawer) 간 스타일 편차를 줄일 수 있다.
  - 휴지통을 스페이스 계층 데이터와 분리하면서도 트리 컨텍스트 안에서 자연스럽게 노출할 수 있다.

### 이동/복사 대상 선택에서 Space 루트 선택 차단 해제 (2026-02-23)
- **문제**:
  - 대상 모달에서 Space 루트는 내부 경로값이 빈 문자열(`''`)이라, 검증 로직이 이를 "미선택"으로 처리해 확인 버튼이 항상 막혔다.
- **결정**:
  - 선택 여부 판단을 `path 비어있음`이 아니라 `space 선택 여부 또는 path 존재` 기준으로 변경한다.
  - 루트 선택(`space_name/`)도 선택 경로 UI에 표시한다.
- **이유**:
  - Space 루트는 정상적인 대상 디렉토리이며, 현재 검증은 데이터 표현(`''`)을 상태 의미(미선택)와 혼동한 버그였기 때문이다.

### Space 쿼터/사용량 1차 구현 전략 확정 (#125, 2026-02-22)
- **문제**:
  - Space별 저장 한도(쿼터)와 실사용량을 운영 화면에서 확인/제어할 수 없어 용량 통제가 불가능했다.
  - 업로드/복사/이동/폴더생성 경로에 쿼터 차단이 없어 초과 상태를 방지할 수 없었다.
- **결정**:
  - DB `space` 테이블에 `quota_bytes`(nullable, `NULL=무제한`) 컬럼을 추가한다.
  - 기존 설치본 호환을 위해 스키마 DDL 수정 + 런타임 마이그레이션(`ALTER TABLE`)을 함께 적용한다.
  - 백엔드에 `QuotaService`를 도입한다.
    - 사용량 계산: Space 루트 재귀 스캔(파일 크기 합)
    - 성능: 짧은 TTL 캐시(10초) + write 성공 시 캐시 invalidate
  - API를 추가한다.
    - `GET /api/spaces/usage`: 접근 가능한 Space별 `used/quota/over/scannedAt` 반환
    - `PATCH /api/spaces/{id}/quota`: 쿼터 설정/해제
  - 쓰기 경로 차단 정책:
    - 목적지 Space 기준으로 projected delta를 계산해 초과 시 차단
    - 단일 요청은 `507 Insufficient Storage`, 배치 항목 실패는 `code=quota_exceeded`
  - Settings > 서버 섹션에 `Space 쿼터/사용량` 대시보드를 추가한다.
    - Space별 사용량/쿼터/사용률 표시
    - 인라인 쿼터 저장 및 무제한 전환 제공
- **이유**:
  - 1차에서는 구조 단순성과 안정성을 우선해 “스캔 + 캐시 + invalidate”로 구현하는 것이 리스크 대비 효과가 가장 크다.
  - projected delta 기반 차단은 기존 이동/복사 충돌 정책 구조와 자연스럽게 결합되어 회귀 범위를 최소화한다.
  - UI를 Settings에 배치하면 운영자 제어 동선을 기존 서버 관리 플로우 안에서 유지할 수 있다.

### Space 쿼터/사용량 설정 섹션 분리 + 권한 재정렬 (2026-02-23)
- **문제**:
  - 쿼터 대시보드가 `Settings > 서버`에 포함되어 있어 서버 설정 권한(`server.config.*`)과 Space 운영 권한(`space.*`) 경계가 모호했다.
  - Space 사용량은 확인만 가능한 사용자(`space.read`)와 쿼터를 수정하는 사용자(`space.write`)를 UI에서 구분하지 못했다.
- **결정**:
  - Settings 내에 `스페이스` 전용 섹션을 추가하고, 쿼터/사용량 대시보드를 서버 섹션에서 분리한다.
  - 섹션 메뉴 노출/접근은 `space.read || space.write`로 제어한다.
  - 쿼터 편집 컨트롤(`저장`, `무제한`, 입력값 수정)은 `space.write`에서만 활성화한다.
  - `서버` 섹션은 서버/WebDAV/SFTP 설정만 유지한다.
- **이유**:
  - 권한 도메인(`server` vs `space`)을 UI 구조와 일치시키면 운영 역할 분리가 명확해지고 오권한 노출을 줄일 수 있다.
  - 조회/수정 권한 분리는 RBAC 정책을 화면에서도 일관되게 반영해 감사성과 예측 가능성을 높인다.

### Space 쿼터 입력 단위 MB 전환 (2026-02-23)
- **문제**:
  - 운영자가 쿼터를 세밀하게 조정하려면 `GB` 단위 입력보다 `MB` 단위 입력이 더 직관적이었다.
- **결정**:
  - 스페이스 설정의 쿼터 입력 단위를 `GB`에서 `MB`로 변경한다.
  - 프론트 변환 상수도 `BYTES_PER_GB`에서 `BYTES_PER_MB`로 전환한다.
- **이유**:
  - 소용량 테스트/운영 구간에서 입력 해상도를 높이고 단위 해석 혼선을 줄일 수 있기 때문이다.

### 휴지통 내부 디렉토리(`.cohesion_trash`) 접근 차단 정책 보강 (2026-02-22)
- **문제**:
  - 숨김 디렉토리 필터가 "엔트리 이름이 점(`.`)으로 시작하는지" 기준이라, API에 `path=.cohesion_trash/...`를 직접 넣으면 일반 browse/download 경로로 휴지통 내부 파일에 접근할 수 있었다.
- **결정**:
  - 사용자 입력 경로에 `.cohesion_trash` 세그먼트가 포함되면 공통적으로 `403`을 반환한다.
  - 일반 파일 API뿐 아니라 browse, 이동/복사 source/destination, 다중 다운로드 경로까지 동일 정책을 적용한다.
  - 이름 변경/폴더 생성 시 예약 이름 `.cohesion_trash`도 차단한다.
- **이유**:
  - 휴지통은 전용 API(`trash/list/restore/delete/empty`)로만 접근되어야 하며, 일반 파일 API 경로 우회가 가능하면 권한/동작 경계가 무너진다.
  - 경로 입력 지점별로 동일한 예약 디렉토리 규칙을 강제해야 API 계약이 단순하고 안전하다.

### 휴지통(Soft Delete) 구현 방식 확정 (#123, 2026-02-22)
- **문제**:
  - 기존 삭제는 즉시 영구삭제라 복구 경로가 없어 운영 리스크가 컸다.
  - 복원 시 경로 충돌 처리 기준이 없으면 사용자가 수동 정리를 반복해야 한다.
- **결정**:
  - 삭제 동작은 파일시스템 숨김 디렉토리(`.cohesion_trash`)로 이동하고, DB(`trash_items`)에 메타데이터를 저장한다.
    - 저장 필드: `space_id`, `original_path`, `storage_path`, `item_name`, `is_dir`, `item_size`, `deleted_by`, `deleted_at`
  - 복원/영구삭제/비우기 API를 분리해 운영 액션을 명시한다.
  - 프론트는 전용 휴지통 모달을 통해 목록 조회/복원/영구삭제/비우기를 처리한다.
  - 복원 충돌은 `overwrite|rename|skip` 정책을 사용자 선택으로 처리한다.
- **이유**:
  - 기존 파일 탐색 UX를 크게 바꾸지 않고 안전한 삭제 경로를 추가할 수 있다.
  - 저장 경로(`storage_path`)와 원래 경로(`original_path`)를 분리하면 복원/정리 작업의 추적 가능성이 높아진다.
  - 충돌 정책을 API 계약에 포함하면 프론트 문자열 파싱 없이 일관 처리할 수 있다.

### 휴지통 진입점 배치 2차: 트리 노드 -> 사이드패널 하단 고정 + 전역 휴지통 (2026-02-22)
- **문제**:
  - 트리 루트에 둔 휴지통 노드는 Space 트리 항목과 성격이 달라 아이콘/정렬 일관성이 떨어지고, 선택 상태 의미도 모호했다.
  - `/trash`가 단일 Space 기준이면 "전체 Space 휴지통 한 번에 관리" 요구를 충족하지 못한다.
- **결정**:
  - 트리 내부 휴지통 노드를 제거하고, 사이드패널 최하단 고정 버튼으로 분리한다.
  - `/trash`는 연결된 모든 Space의 휴지통을 병렬 조회해 병합한 전역 화면으로 동작시킨다.
    - 복원/영구삭제: 선택 항목을 Space별로 그룹화해 각 Space API 호출
    - 비우기: 항목이 있는 모든 Space 대상으로 실행
  - `/trash` 진입 시 왼쪽 Space 트리 선택은 비워서 전역 화면과 트리 선택 상태가 충돌하지 않게 한다.
- **이유**:
  - "트리 = Space/폴더 계층", "휴지통 = 전역 액션 진입점" 역할을 분리하면 UI 의미가 명확해진다.
  - 전역 병합 구조로 바꾸면 사용자가 Space를 바꿔가며 반복 정리할 필요 없이 한 화면에서 관리할 수 있다.

### 이동/복사 충돌 정책 2차: API 정책 확장 + 프론트 대화형 재시도 (2026-02-22)
- **문제**:
  - `move/copy`는 동일 이름 충돌 시 항상 실패(`Destination path already exists`)만 반환해 사용자가 수동으로 파일명을 바꾸거나 재시도해야 했다.
  - 다중 항목 작업에서 충돌이 섞이면 반복 작업 비용이 커지고, 항목별 처리 기준을 일관되게 적용하기 어려웠다.
- **결정**:
  - 백엔드 `move/copy` API에 `conflictPolicy`(`overwrite|rename|skip`)를 추가한다.
  - 충돌 실패 응답에 `code` 필드(`destination_exists`)를 포함해 프론트가 안정적으로 충돌 타입을 식별하게 한다.
  - 프론트는 1차 요청(정책 없음)으로 충돌 목록을 수집하고, 첫 충돌에서 정책을 묻는다.
    - 선택 정책: `overwrite|rename|skip`
    - 적용 범위: `현재 항목만` 또는 `남은 충돌 전체 적용`
  - 정책 적용 재요청은 충돌 항목에 대해서만 수행한다.
- **이유**:
  - 기존 API 흐름(부분 성공 + 실패 목록)을 유지하면서도 덮어쓰기/이름변경/건너뛰기 UX를 추가할 수 있다.
  - `code` 기반 분기로 로케일/문구 변경에 취약한 문자열 비교를 제거할 수 있다.
  - 대량 작업에서는 일괄 적용으로 효율을 확보하고, 필요 시 항목별 판단(현재 항목만)도 지원할 수 있다.

### 이동/복사 충돌 후속 안정화: 동일경로 보호 + 실패 시 UI 동기화 (2026-02-22)
- **문제**:
  - `copy`에서 동일 경로(`source == destination`)에 `overwrite`를 적용하면 대상을 먼저 삭제하는 흐름 때문에 원본 유실 위험이 있었다.
  - 프론트는 성공 건수 기준으로만 refresh/invalidate를 수행해, 실패 중 일부 파일시스템 변경이 발생한 경우 화면이 stale 될 수 있었다.
- **결정**:
  - 백엔드 `copy`에 `same_destination` 가드를 추가해 동일 경로 요청은 항상 실패로 처리한다.
  - 백엔드 `move/copy overwrite`는 대상을 즉시 삭제하지 않고 `destination -> backup`, `source/stage -> destination` 순서의 swap 절차를 사용한다.
    - 중간 단계 실패 시 backup을 원위치로 복구해 기존 대상 유실을 방지한다.
  - 프론트 이동/복사 후처리는 `성공>0` 뿐 아니라 `실패>0`에도 refresh/invalidate를 수행한다.
  - 프론트 충돌 처리 루프에서 사용자 `중단` 시 남은 충돌 큐를 미처리 실패로 집계해 결과 요약이 실제 처리 상태와 일치하도록 조정한다.
- **이유**:
  - overwrite 처리의 안전성(데이터 유실 방지)을 `move/copy` 모두에서 일관되게 보장하기 위해서다.
  - 부분 실패 시점의 실제 파일시스템 상태와 UI 상태를 빠르게 재동기화해 사용자 혼란을 줄이기 위해서다.

### 업로드 충돌 정책(#124) 배치 적용 방식 확정 (2026-02-22)
- **문제**:
  - 기존 업로드는 충돌 시 `overwrite` 중심 동작만 제공해 대량 업로드에서 사용자가 파일별로 반복 대응해야 했다.
  - 충돌 파일을 유지하면서 나머지 파일만 처리하는 `skip`, 자동 파일명 보정 `rename` 정책이 없어 작업 중단/재시도 비용이 컸다.
- **결정**:
  - 업로드 API에 `conflictPolicy`(`overwrite|rename|skip`)를 추가한다.
  - 정책 미지정 + 충돌은 기존과 동일하게 `409`를 유지해 하위호환을 보장한다.
  - 레거시 클라이언트 호환을 위해 `overwrite=true` 파라미터를 계속 허용한다.
  - 프론트는 배치 업로드 중 첫 충돌에서 정책을 1회 선택하고, 동일 배치의 나머지 충돌 파일에 같은 정책을 재사용한다.
  - `rename`은 서버가 `name (n).ext` 규칙으로 비충돌 파일명을 결정하고, `skip`은 `status: skipped` 응답으로 집계 가능하게 처리한다.
- **이유**:
  - 정책을 배치 단위로 고정하면 대량 업로드의 반복 인터랙션을 줄이고 처리 예측 가능성을 높일 수 있다.
  - 충돌 해소 책임을 서버로 통일하면 클라이언트 간 동작 차이를 줄이고 API 계약을 단순하게 유지할 수 있다.
  - 기본 `409` 및 `overwrite=true` 유지로 기존 클라이언트 회귀를 방지할 수 있다.
...
### FTP/SMB 확장 정책 재정의 (2026-02-26, #159)
- **문제**:
  - 사용자 요구가 "OS 네트워크 드라이브로 바로 연결"에 집중되면서, 기존 `FTP 제거 + SFTP 단일화` 결정과 충돌이 발생했다.
  - HTTPS가 없는 운영 환경에서는 WebDAV만으로 동일 UX를 보장하기 어렵다.
- **결정**:
  - 기본 서비스 범위는 `WEB/WebDAV/SFTP`를 유지한다.
  - FTP는 **옵트인(기본 비활성)** 조건으로만 재도입한다.
    - 운영자가 명시적으로 활성화한 경우에만 노출
    - 설정/UI에 평문 프로토콜 보안 경고를 고정 노출
    - 로컬망/신뢰 구간 중심 사용을 기본 가정
  - SMB는 Cohesion 코어에 내장하지 않고 **외부 게이트웨이(Samba) 방식**을 1차 공식 경로로 채택한다.
  - 단계 계획은 `#159 (정책 확정) -> #160 (FTP 1차) -> #161 (SMB 게이트웨이 1차)`로 고정한다.
  - 프로토콜 확장 시에도 Cohesion 계정/Space 권한 모델과 충돌하지 않는 접근 제어를 유지한다.
- **이유**:
  - 사용자 요구(네트워크 드라이브 UX)를 수용하면서도 코어 서버의 공격 표면과 운영 복잡도 급증을 억제할 수 있다.
  - SMB 내장 구현보다 게이트웨이 방식이 플랫폼별 운영/디버깅 리스크가 낮다.

### FTP 서버 재도입 1차 적용 (2026-02-26, #160)
- **문제**:
  - 정책(#159)에서 FTP를 옵션형으로 재도입하기로 했지만, 실제 런타임/설정/UI 경로가 없는 상태였다.
- **결정**:
  - 백엔드에 `internal/ftp` 모듈을 복원하고 서버 lifecycle(`main`)에 start/stop 연동을 추가한다.
  - 설정 모델/기본 YAML에 `ftp_enabled`, `ftp_port`를 다시 도입하되 기본값은 `ftp_enabled=false`로 유지한다.
  - 설정 검증은 아래를 강제한다.
    - `ftp_enabled=true`일 때 `ftp_port` 1~65535
    - `ftp_port != server.port`
    - `ftp_enabled && sftp_enabled`일 때 `ftp_port != sftp_port`
  - 상태 API와 상태 팝오버 프로토콜 순서에 FTP를 복원한다. (`WEB -> WebDAV -> FTP -> SFTP`)
  - 설정 화면에 FTP 카드(활성화/포트)를 추가하고 평문 보안 경고를 노출한다.
- **이유**:
  - 운영자가 필요 시에만 FTP를 노출하는 정책을 실제 동작으로 반영하면서, 기본 보안 자세(비활성)를 유지하기 위함.

### SMB 게이트웨이 1차 적용 (2026-02-27, #161)
- **문제**:
  - 정책(#159)에서 SMB를 외부 게이트웨이로 확정했지만, 상태 UI/설정 UI/운영 문서가 없어 실제 운영자가 재현 가능한 기준이 부족했다.
  - FTP 서비스 시작 성공 판정이 고정 지연(200ms)에 의존해, 포트 충돌 시 오탐 가능성이 있었다.
- **결정**:
  - `/api/status`에 `smb` 프로토콜 상태를 추가하고, 상태값을 `external`로 정의해 코어 내장형이 아님을 명시한다.
  - 서버 상태 팝오버에 SMB 라벨/순서를 반영하고(`WEB -> WebDAV -> FTP -> SFTP -> SMB`), `external` 상태 번역을 추가한다.
  - Settings > 서버에 SMB 게이트웨이 안내 카드를 추가하고, Windows/macOS 연결 경로 및 운영 가이드 문서를 노출한다.
  - 운영 문서 `docs/smb_gateway.md`를 추가해 Samba 예시, 보안 가드(guest 금지/기본 read-only), 검증 절차를 표준화한다.
  - FTP 시작 로직은 포트 프로빙 + settle 재확인 방식으로 보강하고, 경계 테스트를 추가한다.
  - 개발 기본 config의 `ftp_enabled`를 `false`로 유지해 옵트인 정책을 보장한다.
- **이유**:
  - SMB 지원 모델의 핵심은 내장 구현이 아니라 운영 가이드 가능한 게이트웨이 경로 확정이며, 상태/UI/문서를 함께 제공해야 재현성이 확보된다.
  - FTP 시작 안정성 보강과 기본 비활성 정책 유지로 보안/운영 리스크를 동시에 낮출 수 있다.

### SMB 게이트웨이 노출 보류 (2026-02-27, #161 후속)
- **문제**:
  - #161에서 SMB 게이트웨이 1차 작업을 완료했지만,
  - SMB가 코어 내장 구현이 아닌 상태에서 안내 중심 노출은 사용자 기대(즉시 사용 가능)와 차이가 발생했다.
  - 설정 화면/상태 팝오버의 SMB 표시는 실제 기능 제공 범위를 과대 인식하게 만들 수 있었다.
- **결정**:
  - 제품 UI/상태/README에서 SMB 노출을 제거한다.
    - `/api/status`의 `smb` 항목 제거
    - 상태 팝오버의 SMB 라벨/`external` 상태 제거
    - Settings > 서버 SMB 안내 카드 제거
    - `docs/smb_gateway.md` 제거
  - SMB는 향후 자체 구현(또는 명확한 제품 범위 재정의) 전까지 보류한다.
- **이유**:

...
## 개발 프로세스
### Sass 공통 레이아웃 스타일 도입 (2026-02-15)
- **문제**:
  - 페이지/컴포넌트별 인라인 레이아웃 스타일이 분산되어 여백/정렬 규칙 일관성이 흔들릴 수 있음.
  - Main/Settings/Browse 레이아웃 간 유사한 구조를 반복 정의하고 있어 유지보수 비용이 큼.
- **결정**:
  - `assets/scss`에 토큰/믹스인/전역 유틸 클래스를 도입한다.
  - `MainLayout`, `MainSider`, `Settings`, `FileExplorer`의 반복 스타일은 `ui-*` 공통 클래스로 치환한다.
  - 전역 진입 스타일은 `global.css` 대신 `global.scss`로 전환하고 `sass`를 devDependency로 추가한다.
- **이유**:
  - 레이아웃 규칙 변경 시 수정 지점을 공통 레이어로 모아 유지보수 효율을 높일 수 있다.
  - 페이지 간 간격/정렬 리듬을 일관되게 유지할 수 있다.
